  ...
  // TODO: Get each tagged player, get its dimension, work with that
	...
	for (let y = 0; y < field_frames[current_frame].length; y++) {
		for (let x = 0; x < field_frames[current_frame][y].length; x++) {
			...
			if (field_frames[next_frame][y][x] != cell_state) {
				block = dimension.getBlock( {x: field_origin.x+x, y:field_origin.y+y, z:field_origin.z}).setPermutation(cells[field_frames[next_frame][y][x]]);
			};
